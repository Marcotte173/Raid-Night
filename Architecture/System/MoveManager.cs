...
public class MoveManager : MonoBehaviour
{
    ...
    public bool GetNewPath(Character g,Tile end,bool direct)
    {
        ...
        if (move.unwalkable.Contains(start)) move.unwalkable.Remove(start);
        while (!move.closed.Contains(end))
        {
            foreach (Tile neighbor in current.neighbor)
            {
                ...
                neighbor.h = FindTile.instance.Distance(neighbor.transform.position, end.transform.position);
    ...
    {
        if (move.character.target == null || move.character.target.ko) StopMoving(move);
        else MoveAgent(move, (move.character.target.move.isMoving) ? move.character.target.move.nextTile : move.character.target.move.currentTile);
    }
    ...
    public void MoveAgent(Move move, Tile target,bool direct)
    {
        ...
        else if (move.nextTile != null) Move(move);
        else StopMoving(move);
    }

    public void StopMoving(Move move)
    {
